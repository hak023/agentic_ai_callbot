...
기존 콜센터 자동화는 보통 **시나리오를 먼저 설계하고, 그 시나리오 안에서 고객을 유도하는 방식**으로 진행됐습니다. FAQ를 만들고, 인텐트를 정의하고, ARS 메뉴를 구성한 뒤, 고객 발화를 그중 하나에 맞추는 구조입니다.

이 방식은 초기 구축에는 명확해 보이지만 운영 단계에서 한계가 빠르게 드러납니다.

- 새로운 정책이나 공지가 생기면 FAQ와 시나리오를 다시 고쳐야 합니다.
- 고객이 “예약 변경하면서 주차도 물어보는” 식으로 복합 요청을 하면 메뉴형 흐름이 흔들립니다.
- 상담원이 현장에서 해결한 지식은 통화가 끝나면 사라지고, 다음 AI 응대에 자동 반영되지 않습니다.
- AI가 모르는 질문을 만났을 때 전화, 웹, 문자, 담당자 전환이 서로 분리되어 고객 경험이 끊깁니다.
...
| 질문 | Ideation |
|------|----------|
| **지식을 미리 다 구축하지 않고 시작할 수 있을까?** | 통화·문자·운영자 답변을 지식으로 전환하면, 운영 자체가 학습 데이터가 됩니다. |
| **ARS 메뉴 대신 고객 목적을 바로 실행할 수 있을까?** | 고객 발화를 의도와 업무 목표로 해석하고, 필요한 도구를 AI가 선택하게 합니다. |
| **AI가 틀릴 위험을 줄이면서도 상담원 부담을 낮출 수 있을까?** | 낮은 신뢰도나 민감 질문은 HITL로 넘기고, 사람의 답변을 다시 지식으로 환류합니다. |

즉, 목표는 “정해진 시나리오를 잘 따라가는 봇”이 아니라 **전화·문자·예약·상담원 판단을 하나의 업무 그래프로 실행하는 Agentic PBX**였습니다.

### 어떤 기술 방향을 수립했는가

이 Ideation은 다음 기술 방향으로 정리됐습니다.

| 방향 | 선택한 기술·구조 |
|------|------------------|
| 통화 제어는 표준 PBX처럼 안정적으로 | Python 기반 SIP B2BUA, RTP Relay, 착신 정책 |
| 음성 대화는 실시간 턴교환에 맞게 | Pipecat, VAD, STT/TTS, 스마트 바지인, 스트리밍 처리 |
| 고객 의도는 행동으로 연결 | LangGraph, intent routing, LangChain Tool Use |
| 지식은 운영 중 계속 성장 | ChromaDB, Active RAG, 시맨틱 캐시, 문서·통화·HITL 적재 |
| 조직별 AI는 완전히 분리 | owner 기반 멀티테넌트 컬렉션, 페르소나, 예약·착신 DB 격리 |
| 사람 개입은 전화 흐름 안에 포함 | Socket.IO 대시보드, HITL 승인·수정, TTS/문자 재전달 |

### 구현으로 어떻게 녹였는가

구현에서는 “통화 한 건”이 아래 흐름을 자연스럽게 지나가도록 설계했습니다.

```mermaid
flowchart LR
    In["전화·문자 입력"] --> Control["SIP B2BUA / MESSAGE"]
    Control --> AI["STT 또는 텍스트 에이전트"]
    AI --> Intent["LangGraph 의도 분류"]
    Intent --> RAG["테넌트별 RAG·캐시"]
    Intent --> Tool["예약·호전환·캠페인 도구"]
    Intent --> HITL["운영자 HITL"]
    RAG --> Reply["TTS·문자 응답"]
    Tool --> Reply
    HITL --> Reply
    Reply --> Learn["통화·문자·답변 지식화"]
```

고객이 전화를 걸면 B2BUA가 착신 정책을 판단합니다. 사람이 받을 수 있으면 사람에게 연결하고, 무응답이나 야간이면 AI가 인수합니다. 고객 발화는 STT를 거쳐 LangGraph로 들어가고, 의도에 따라 지식 검색, 예약 도구, 호전환, HITL, 문자 후속 흐름으로 분기됩니다.

문자 문의는 SIP MESSAGE로 들어와 STT 없이 같은 에이전트와 지식 체계를 사용합니다. 아웃바운드 캠페인은 반대로 시스템이 먼저 전화를 걸고, 미션과 질문을 바탕으로 고객 응답을 수집합니다.

### 기대 효과

- **도입 장벽 감소**: 모든 FAQ와 시나리오를 완성한 뒤 시작하지 않아도, 문서 업로드와 운영 중 대화로 지식이 쌓입니다.
- **고객 경험 개선**: 고객은 메뉴를 누르는 대신 자연어로 말하고, AI는 지식 검색·예약·전환 같은 행동까지 이어갑니다.
- **상담원 부담 감소**: 상담원은 모든 통화를 직접 받지 않고, AI가 어려워하는 구간에만 짧게 개입합니다.
- **품질 선순환**: HITL 답변, 통화 이력, 문자 문의가 다음 응대의 지식으로 반영되어 자동화율이 높아집니다.
- **운영 확장성 확보**: 테넌트별 지식·페르소나·정책이 분리되어 한 플랫폼에서 여러 조직의 “나만의 AI”를 운영할 수 있습니다.
