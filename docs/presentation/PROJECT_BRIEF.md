...
| 용어 | 설명 |
|------|------|
| **SIP / SIP B2BUA** | 인터넷 전화의 표준 신호 프로토콜과, 양쪽 통화를 중간에서 제어·중계하는 서버 역할을 의미합니다. |
| **RTP** | 통화 음성을 실시간으로 주고받는 표준 미디어 전송 프로토콜입니다. |
| **VAD (Voice Activity Detection)** | 사람이 말하기 시작했는지·끝냈는지를 판단하는 음성 활동 감지 기술입니다. |
| **STT / TTS** | 음성을 텍스트로 바꾸는 기술(STT)과 텍스트를 음성으로 합성하는 기술(TTS)입니다. |
| **LLM** | 대규모 언어 모델. 본 시스템은 Google Gemini 2.5 계열을 사용합니다. |
| **RAG (Retrieval-Augmented Generation)** | 외부 지식을 검색해 LLM 답변의 근거로 사용하는 방식입니다. |
| **Active RAG** | 통화·운영자 답변·문서가 자동으로 지식이 되어 RAG 품질이 시간과 함께 자라는 운영 방식을 가리킵니다. |
| **ChromaDB** | 의미 기반 검색을 위한 벡터 데이터베이스입니다. 조직(테넌트)별로 격리해 사용합니다. |
| **LangGraph** | 의도 분류와 다단계 행동 결정을 그래프로 표현·실행하는 에이전트 오케스트레이션 프레임워크입니다. |
| **HITL (Human-in-the-Loop)** | AI가 자신 없을 때 운영자가 짧게 개입하는 사람-개입 안전장치입니다. |
| **테넌트(Tenant)** | 한 플랫폼에서 데이터·정책·지식이 완전히 분리된 단위 조직(예: 내선번호별 매장)입니다. |
| **CID (Caller Identification)** | 발신자 식별 정보. 본 시스템은 통화 시 도크 형태로 발신자 카드와 통계를 표시합니다. |
| **Pipecat** | 실시간 음성 AI 파이프라인을 구성하는 오픈소스 프레임워크로, 본 시스템 음성 처리 체인의 기반입니다. |
