...
| 파일 | 역할 |
|------|------|
| **본 문서 (`prd.md`)** | **마스터 PRD**: SIP Core + AI Phase 개요, Cross-cutting, 지표, **사업·프로젝트 요약**, **AI Call Agent MM(서버 역할·Epic Phase, 합계 동일 72.0)** |
...
### 문서 통합 이력
- **v3.6** (2026-05-12): **AI Runtime** 공수 **15.0 → 22.0 MM** — Pipecat(또는 동등) **실시간 음성 파이프라인**, **Intent·시나리오별 분리·회귀**, 스트림·세션 경계 검증 반영. **합계 72.0 MM** — [production-deployment-architecture.md](../architecture/production-deployment-architecture.md) **§11.4** 와 동기.
- **v3.5** (2026-05-11): 상용 문서 **§11.4** SW 개발비와 PRD MM 정합을 문서 본문에 명시.
...
- **미포함**: PM 전담, 별도 QA 조직, UAT, **가동 후 월간 유지보수·전력·랙 등 운영 OPEX** — 본 PRD MM은 **순수 SW 개발**에 대응하며, 상용 문서 [production-deployment-architecture.md](../architecture/production-deployment-architecture.md) **§11.4** 금액과 정합한다. (상용 문서 **§11절** 은 **하드웨어 CAPEX·SW 개발·연동·참고 견적**까지만 다루며 **월간 OPEX 산정은 범위 밖**이다.)

### 산정 전제 (Epic·서버 합계 일치)

이전 **Epic만 합산한 34 MM**은 온프레미스 전용 **STT/TTS/LLM** 추론 스택, **PostgreSQL·Qdrant** 운영 반영, **코어(WT·바이토·유엔젤) 연동 검증**, **250 세션·2.1 CPS** 부하 리허설 등을 충분히 넣지 않은 값이었다. **v3.2~v3.3**에서 이를 **44.0 MM**으로 정렬했으나, 검토 결과 **추론 품질 보증·런타임 복잡도·다층 검증**을 상용 **품질 개런티** 수준에 두면 ROM이 부족하다는 판단이 있었다. **v3.4**에서는 아래 **가산 근거**를 반영하여 **합계 65.0 MM**으로 상향했다. **v3.6**에서는 상용 아키텍처 검토에 따라 **AI Runtime**에 Pipecat·시나리오 분리 등을 반영해 **+7.0 MM** 가산, **합계 72.0 MM**이다. **Epic 표**와 **서버 역할 표**는 동일 전제에서 **분해만 다르다**.
...
| **AI Runtime (v3.6 가산)** | **Pipecat**(또는 동등) 기반 **실시간 음성 파이프라인**(프레임/프로세서·트랜스포트·턴·바지 인터럽션), **Intent·업무 시나리오별** 모듈·프롬프트·**시나리오 단위 회귀**, STT/TTS·GW 해시·세션 고정과의 **스트림 정합** 검증에 대한 **+7.0 MM** |
...
### AI Call Agent 서버별 MM (배포 역할 축, 합계 72.0 MM)
...
| **AI Runtime** | 세션 오케스트레이션, Intent·RAG·HITL, STT/TTS/LLM **클라이언트**, 정책·도구 호출, WT 재생 시그널, DB 접근 **+ 복잡 경로·복구·운영 경계 개런티** **+ Pipecat·실시간 음성 파이프라인·Intent 시나리오별 분리·스트림 정합(v3.6)** | **22.0** |
...
### 기능(Epic·Phase)별 MM (기능 축, 합계 72.0 MM · 서버 표와 동일 총액)
...
| **소계 Phase 1** | | **19.0** |
| Intent·NL IV·대화 상태·오케스트레이션 (**런타임 복잡도·경계 조건·Pipecat 음성·시나리오 분리 v3.6**) | Epic 2.1 | **13.0** |
...
| **합계 (Epic·Phase, 상용 전제)** | | **72.0** |

**Epic 합계 = 서버 역할 합계 = 72.0 MM** — 하나는 **기능·Phase** 축, 하나는 **배포 노드** 축일 뿐이다.
...
| **기존 코어·외부 서버의 제품 개발** | 통화매니저 AS↔WTIMS **호 세션 릴레이**, WTIMS의 통합 시그널 발신·**RTP 미러**, 유엔젤·바이토 서버의 **AI 호출·조회 REST** 추가 등 — 문서상 **AI Call Agent 제외** 연동 과제 | **아니오** — 상용 문서 **§11.3**에서 별도 공수·금액 산정. 본 **72 MM**과 **이중 계상 금지**(본문 위 bullet과 동일) |
...
- **기존 노드와의 연동**: **AI 측에서 구현·검증하는 범위**는 본 MM에 들어 있으나, **CM·WT·유엔젤·바이토 제품 코드 변경**은 §11.3 **별도 MM**이다. 상용 전체 일정은 **72 MM + §11.3 연동 MM**으로 보는 것이 맞다.
