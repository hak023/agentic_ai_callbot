...
**500호 부하 시 합산 검증(참조 문서와 동일):** STT **약 900** 동시 스트림(500×1.8), LLM **약 15.3 QPS**(운영 상한 **40 QPS** 대비 여유), Runtime **500** 세션(벤치와 동일 → **실효 상한**).
...
| **NLP·LLM** *(동일 호스트)* | 추론 QPS | **50** (운영 **40**) | 평균 **≈0.031 QPS/호** · AI **0.12** · 유저간 **≈0.008** | **1,304호** | 39,120명 | **2** All-Active |
| **PostgreSQL·Qdrant** *(동거)* | TPS / 검색 QPS | PG·Vector 각 벤치 별도 | 경로별(§6.1·§7) — 호당 단일치 비권장 | 500호 부하 시 **여유**(참조 §6.4) | — | **2** P+S |

> **해석:** 표의 **최대 수용 동시호**는 노드별 **단독 역산**이다. 본 구성에서는 **AI Runtime 500호**가 **실효 상한**이며, STT(694호)·LLM(1,304호)은 그보다 크다. 호당 **1.0 스트림**이면 STT 단독 한계는 **약 1,250동시 통화**까지 상승(다른 병목 전제). **1차 NLP**는 LLM 서버 CPU 부하로 소량 반영(벤치 LLM 여유 내).
...
[prd.md](../product/prd.md) **기능(Epic·Phase)별 MM** 및 본 문서 **§1** 범위를 맞춰 **55.0 MM**으로 재산정했다(TTS·NL-IVR·부재중 응대·Phase 4 등 **제외**). 상세 근거·Epic 매핑은 **§5.5**를 본다.

#### 기능별 (§1)

| # | 기능 | 담당 서버 | MM |
|---|------|-----------|-----|
| 1 | **폭언·욕설 실시간 감지 및 호 종료 안내** | STT, NLP·LLM, AI Runtime, DB | **20.0** |
| 2 | **실시간 통화 자막** | STT, AI Runtime | **7.0** |
| 3 | **실시간 대화 TIP** | DB, NLP·LLM, AI Runtime | **6.0** |
| 4 | **주의·블랙·스팸 사전 알림** | DB, NLP·LLM, AI Runtime | **4.0** |
| 5 | **자동 연락처 등록** | DB, NLP·LLM, AI Runtime | **3.0** |
| 6 | **CID 맥락·의도** | DB, NLP·LLM, AI Runtime | **5.0** |
| | **기능 배분 소계** | | **45.0** |
| — | **공통·통합·검증**(시뮬레이터·E2E·§3.2 부하) | AI Runtime, STT, NLP·LLM, DB | **10.0** |
| | **합계** | | **55.0** |
...
| 서버 | MM | 주요 담당 기능 (#) |
|------|-----|-------------------|
| **AI Runtime** *(API·세션 통합)* | **19.0** | #1~#6, 공통 |
| **STT** | **9.0** | #1, #2, 공통 |
| **NLP·LLM** *(동일 호스트)* | **12.0** | #1, #3, #4, #5, #6, 공통 |
| **DB** *(PostgreSQL·VectorDB 동거)* | **5.0** | #1, #3~#6, 공통 |
| **합계** | **55.0** | |
...
| | **기존 노드 권장 범위** | | **약 2.1억 ~ 2.8억 원** | §11.3 미산출 항목 ROM 반영 시 |
...
| 기존 노드·연동 (§5.3) | **약 2.45억 원** *(권장 **2.1억 ~ 2.8억 원**)* |
| **SW 개발 합계** | **약 9.6억 원** *(권장 **약 9.25억 ~ 10.0억 원**)* |

### 5.5 CAPEX + 초기 개발비 요약

| 구분 | ROM |
|------|-----|
| **CAPEX** (§4, 서버·HW 8대) | 약 **2.27억 원** (권장 **1.8억 ~ 2.7억 원**) |
| **초기 SW 개발** (§5.4) | 약 **9.6억 원** (권장 **9.25억 ~ 10.0억 원**) |
| **합계 (CAPEX + 초기 개발)** | 약 **11.9억 원** (권장 **약 11.0억 ~ 12.7억 원**) |
