...
| 구분 | ROM | 상세 |
|------|-----|------|
| **CAPEX** (서버·HW 8대) | **약 2.27억 원** | §4.1 (권장 **1.8억 ~ 2.7억 원**, ±20%) |
| **초기 SW 개발** | **약 9.6억 원** | AI Call Agent **7.15억** (§5.2) + 기존 노드 **2.45억** (§5.3) |
| **합계** | **약 11.9억 원** | 권장 **약 11.0억 ~ 12.7억 원** |
...
> **DB 노드당 ROM**: 참조 문서 PostgreSQL HA(약 1,662만)와 Qdrant(약 1,090만)를 **동일 호스트**로 합산·소폭 절감한 ROM(스펙은 PostgreSQL HA 기준, VectorDB 워크로드 동거). **CAPEX 합계**는 상단 **「도입 비용 요약」** 참조.
