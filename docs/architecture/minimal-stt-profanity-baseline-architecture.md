...
| **EMS** | §2 **아키텍처 도식 미포함** · CAPEX **3대(2+TB1)** §4.2 · 초기 SW **약 0.30억 가상 전제**(§5.1, **AI Call Agent 합계에 포함**) |
| **기존 활용** | 교환기, 통화매니저 AS, WTIMS, 유엔젤/바이토 API, PC Client(자막·기존 폭언 알림) |
| **성능·용량** | **동시 625호·약 5.21 CPS·시간당 18,750콜** (120초/호, 실효 병목 **STT**, §3.2) |
| **CAPEX·서버 근거** | 상용 문서 §6.3·§11.2 (온프레미스·TTS·DB 제외) · **EMS** 는 §4.2 |
...
**STT** 서버 CAPEX·STT SW는 **별도 산출**이며 SW·CAPEX **본편 합계에 넣지 않는다**. **EMS** 초기 SW는 **약 0.30억(가상)** 으로 §5에 반영하며 **AI Call Agent SW(1.5억)에 포함**한다. EMS **HW CAPEX** 는 §4.2 **별도**(ROM 산정 필요).

| 구분 | ROM | 상세 |
|------|-----|------|
| **CAPEX** (본편 HW **6대**) | **약 1.45억 원** | AI Runtime 3 (A/S+TB) + NLP·LLM 3 (All-Active+TB, §4.1) |
| **CAPEX** (EMS HW **3대**) | **산정 필요** | 최소 구성 2+TB1 (§4.2) |
| **초기 SW 개발** (본편) | **약 3.00억 원** | AI Call Agent **1.50억** (§5.2, EMS SW 포함) + 기존 노드 **1.50억** (§5.3) |
| **합계** (본편) | **약 4.45억 원** | CAPEX(6대) + 초기 SW(본편) · **STT·EMS HW CAPEX는 별도** |
...
**EMS:** 운영·장애 대응을 위해 **관측(EMS) 구축이 필요**하나, 본 최소 구성 **§2 아키텍처 도식에는 그리지 않는다**. **HW CAPEX** 는 §4.2, **초기 SW** 는 **약 0.30억(가상 전제)** 로 §5.1에 두고 **AI Call Agent 개발비(§5.2)에 포함**한다(외부·전사 공용 관제 대체 시 재검토).
...
### 4.2 EMS (관측) — CAPEX 전제 (별도)

상용 문서는 EMS를 **OTel·Metrics·Log·Trace·Alert·Grafana** 등 **다수 프로세스·노드**로 전개한다([production-deployment-architecture.md](./production-deployment-architecture.md) §2·§8). 본 최소 구성은 **물리 서버 수·비용을 최소화**하기 위해 **EMS 전용 호스트 3대(운영 2 + TB 1)** 로 통합 배치하는 전제만 둔다(프로세스 배치·용량은 **별도 설계·스테이징**).

| 서버 종류 | 운영 | TB | **합계** | 이중화·비고 | 권장 스펙(노드당, 요약) | 노드당 ROM | 합계 ROM |
|-----------|------|-----|----------|-------------|-------------------------|------------|----------|
| **EMS** *(관측·별도 구역)* | 2 | 1 | **3** | 운영 All-Active + TB 1 | **최소** 16 vCPU, 64 GB RAM, HDD 1 TB, 1 Gbps *(AI Runtime과 동일 티어)* | **산정 필요** | **산정 필요** |

| 항목 | 본 문서 처리 |
|------|----------------|
| **§2 아키텍처** | EMS **미표기** (코어·AI·연동만) |
| **관측 대상** | AI Runtime, STT, NLP·LLM 및 연동 구간(OTel 등 — 상용 §8 참고) |
| **CAPEX ROM** | **산정 필요** (§11.2 단가·보존기간·로그량·통합 배치 범위) |
| **초기 SW (§5.1)** | **약 0.30억 원(가상)** — OTel·메트릭·로그·트레이스·알람·Grafana 등 · **§5.2 AI Call Agent에 포함** |
| **대안** | 전사·외부 공용 관제로 **대체** 시 위 3대·EMS SW는 **0**으로 재검토 |

> **EMS HW CAPEX** 는 **「도입 비용 요약」본편 합계(4.45억)에 미포함**(§4.2). **EMS 초기 SW(0.30억 가상)** 는 **§5.2·§5.4 본편 SW(3.00억)에 포함**한다.
...
| 구분 | MM | 비고 |
|------|-----|------|
| **AI Call Agent (본편)** | **11.5** | Runtime **5** + NLP·LLM **4** + EMS **≈2.5**(§5.1, **0.30억 가상**) |
| **STT** | *(별도)* | §5.1 표기만, **MM·금액 합계 제외** |
| **기존 노드 연동** | **§5.3** | MM **5.0** + WTIMS **0.85억** · AI MM과 이중 계상 없음 |
...
| **EMS** *(관측·별도 구역)* | **≈2.5** *(가상)* | OTel·메트릭·로그·트레이스·알람·Grafana·AI 노드 연동(§4.2) | **약 0.30억 원** *(가상 전제)* |
| **STT** | **—** | **#1, #2** 입력(전사) — **별도 과제** | *(AI Call Agent 합계 제외)* |
| **NLP·LLM** *(동일 호스트)* | **4.0** | **#1** 1차 NLP **1.0** · 2차 LLM 폭언 확정 **3.0** | **약 0.52억 원** |
| **합계 (AI Call Agent)** | **11.5** | STT 제외 · EMS **0.30억** 포함 | **약 1.50억 원** |

### 5.2 AI Call Agent 개발비 ROM

| 항목 | MM | 단가 | 금액 (ROM) |
|------|-----|------|------------|
| AI Call Agent SW (본편 HW 6대·§1, **EMS SW 포함**) | **11.5** | 약 1,300만 원/MM | **약 1.50억 원** |

> AI 측 **WTIMS·바이토·유엔젤 연동 클라이언트·규약**은 Runtime MM에 포함(폭언·API **2 MM**·자막 **3 MM**). **EMS** 는 **약 0.30억(가상, ≈2.5 MM)** 을 위 합계에 포함한다. **기존 제품 코드 변경**은 §5.3만 반영한다.

### 5.3 기존 노드·연동 개발비 ROM

| # | 노드 | 개발·연동 범위 (본 문서) | MM | 금액 (ROM) | 비고 |
|---|------|--------------------------|-----|------------|------|
| 1 | **WTIMS** | 실시간 STT 대상 호에 **AI Runtime** 세션·**STT** RTP 미러(§2.2) | *(상용 이력)* | **약 0.85억 원** | [production-deployment-architecture.md](./production-deployment-architecture.md) **§11.3** — §5.4 소계 **1.50억** 정합 |
...
| | **기존 노드 소계** | | **5.0** *(+ WTIMS 0.85억)* | **약 1.50억 원** | 유엔젤 1 MM + 바이토·PC 4 MM + WTIMS |

### 5.4 초기 SW 개발비 합계

| 구분 | ROM |
|------|-----|
| AI Call Agent SW (§5.2, EMS 포함) | **약 1.50억 원** |
| 기존 노드·연동 (§5.3) | **약 1.50억 원** |
| **SW 개발 합계 (본편)** | **약 3.00억 원** |

※ **EMS SW(0.30억 가상)** 는 §5.2 **AI Call Agent에 포함**(§5.4 별도 행 없음). **STT SW** 는 별도 과제. CAPEX·전체 합계는 **「도입 비용 요약」** 참조.
...
| EMS·OTel·메트릭·로그·트레이스·알람·Grafana | — | EMS | **≈2.5** | **0.30억 가상** · §4.2 |
| STT·온프레미스 ASR·품질 | #1, #2 | STT | **—** | **별도 과제** |
| **합계 (AI Call Agent)** | | | **11.5** | STT 제외 · §5.2 **1.50억** |
...
| 기능 | 6종 | **2종** (#1, #2) |
| DB 서버 | 2대 | **없음** |
| EMS | 상용 다수 노드 | **§2 미표기** · CAPEX **3대** · SW **0.30억 가상**(AI Call Agent 포함) |
| AI MM | 55 | **11.5** (STT MM 별도) |
| AI Runtime MM | 19 | **5** (자막 릴레이 **3**) |
| NLP·LLM MM | 12 | **4** (폭언만) |
| EMS MM | — | **≈2.5** *(0.30억 가상)* |
| 공통·검증 MM | 10 | **0** (서버별 포함) |
| 유엔젤 연동 | CM+호제어 등 | **WTIMS 릴레이 1 MM** (호제어 기구현) |
| 바이토·PC | 자막+폭언 알림 | **자막 4 MM** (폭언 알림 기구현) |
| SW 개발 합계 | — | **3.00억** (AI **1.50** + 기존 노드 **1.50**) |
