...
| **성능·용량** | **동시 500호·약 4.17 CPS·시간당 15,000콜** (120초/호, §3.2) |
...
벤치 **정본**은 [production-deployment-architecture.md](./production-deployment-architecture.md) **§5.1(STT)·§5.3(LLM)·§5.4(Runtime)** 이다. 본 절은 **최소 구성(폭언·자막)** 기준으로 **서버당 한도 → 호당 부하(120초/호) → CPS·동시 호** 를 역산한다. **운영 2노드/종**(TB 제외). **1차 NLP** 는 LLM과 **동일 호스트**이나 GPU·QPS 병목 산정에서 **제외**(규칙·경량 후보만).
...
| 서버 | 측정 단위 | **노드당** 상한 | **클러스터(운영 2대)** | 비고 |
|------|-----------|-----------------|------------------------|------|
| **STT** | 동시 **ASR 채널** | **625** | **1,250** | 채널 1 = RTP 미러 디코딩 1본 · L40S×1/노드 |
| **NLP·LLM** | LLM **질의 처리량(QPS)** *(NLP 미포함)* | 이론 **12.5** | 이론 **25** / 운영 **80% → 20** | 8B fp8 · 노드 **1,500 tok/s** · **질의당 120 tok** 가정(§5.3) |
| **AI Runtime** | 동시 **세션(스트림)** | **250** | **500** | 제어·API·오케스트레이션 · RTP 비경유 |
...
| **STT** *(노드당)* | **625 ÷ 2** | **312호** | **≈ 2.60** |
| **LLM** | **20 QPS ÷ (1.1/120)** | **≈ 2,180호** | **≈ 18.2** |
| **AI Runtime** | **500 세션 ÷ 1/호** | **500호** | **500÷120 ≈ 4.17** |

#### 실효 상한 (교차 병목)

| 항목 | 값 |
|------|-----|
| **실효 병목** | **AI Runtime** (클러스터 동시 세션 **500**) |
| **실효 동시 호** | **500호** |
| **실효 CPS** | **≈ 4.17** |
| **시간당 콜** | **15,000** *(500×3600/120)* |
| **500호 시 STT** | **1,000 채널** 점유 / 한도 1,250 → **여유** |
| **500호 시 LLM** | **≈ 4.6 QPS** / 운영 20 → **여유** |

> 확장안의 **AI 20%·호당 1.8 STT 채널** 등은 본 최소 구성 §3.2에 **적용하지 않는다**. STT·LLM 수치는 스테이징 **4.17 CPS·500 동시호** 부하로 재확인한다.
