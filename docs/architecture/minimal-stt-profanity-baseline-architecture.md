...
| **성능·용량** | **동시 250호·약 2.08 CPS·시간당 7,500콜** (120초/호, Runtime **Active 1대**, §3.2) |
...
**물리 합계: 9대** (본편 **6대** + STT **3대** 별도). **용량 산정:** Runtime **Active 1대(250세션)**, STT·LLM **운영 2대** 합산.

### 3.2 성능·용량 기준 (가상 시나리오)

벤치 **정본**은 [production-deployment-architecture.md](./production-deployment-architecture.md) **§5.1(STT)·§5.3(LLM)·§5.4(Runtime)** 이다. 본 절은 **최소 구성(폭언·자막)** 기준으로 **서버당 한도 → 호당 부하(120초/호) → CPS·동시 호** 를 역산한다. **AI Runtime** 은 **Active/Standby** 이므로 용량은 **Active 1대**만 본다. **STT·NLP·LLM** 은 **All-Active 2대** 합산(TB 제외). **1차 NLP** 는 LLM 동일 호스트·**용량 산정 제외**.
...
| **Runtime** | **호당 1세션**(오케스트레이션·전사 릴레이·폭언·자막) |
...
| **AI Runtime** | 동시 **세션(스트림)** | **250** | **250** *(**Active 1대**만)* | **Active/Standby** · Standby는 용량 미포함 · RTP 비경유 |
...
| **STT** *(서버 1대)* | **625 ÷ 2** | **312호** | **≈ 2.60** |
| **LLM** | **20 QPS ÷ (1.1/120)** | **≈ 2,180호** | **≈ 18.2** |
| **AI Runtime** | **250 세션 ÷ 1/호** *(Active 1대)* | **250호** | **250÷120 ≈ 2.08** |

#### 실효 상한 (교차 병목)

| 항목 | 값 |
|------|-----|
| **실효 병목** | **AI Runtime** (**Active 1대**, 동시 세션 **250**) |
| **실효 동시 호** | **250호** |
| **실효 CPS** | **≈ 2.08** |
| **시간당 콜** | **7,500** *(250×3600/120)* |
| **250호 시 STT** | **500 채널** / 한도 1,250 → **여유** |
| **250호 시 LLM** | **≈ 2.3 QPS** / 운영 20 → **여유** |

> 확장안의 **AI 20%·호당 1.8 STT 채널** 등은 본 최소 구성 §3.2에 **적용하지 않는다**. STT·LLM 수치는 스테이징 **2.08 CPS·250 동시호** 부하로 재확인한다.
...
**TB(Test Bed) 구성:** 서버 **종류당 1대**를 **TB 전용**으로 추가한다(운영과 **동일 스펙**). 연동·부하·릴리즈 검증용이며, **§3.2 용량(250호 등)에는 TB·Standby 대수를 넣지 않는다.**
