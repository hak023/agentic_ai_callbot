...
| **EMS** | §2 **아키텍처 도식 미포함** · §4 CAPEX **최소 3대(2+TB1)** 별도 반영 · **금액·SW 공수 미산정(고려 필요)** |
...
| **CAPEX** (EMS HW **3대**) | **고려 필요** | 최소 구성 2+TB1 (§4.2) · **도입 비용 합계·본편 CAPEX 합계에 미포함** |
| **초기 SW 개발** (본편) | **약 2.26억 원** | AI Call Agent **1.04억** (§5.2) + 기존 노드 **1.22억** (§5.3) |
| **합계** (본편) | **약 3.71억 원** | CAPEX(6대) + 초기 SW · **EMS CAPEX·EMS SW는 별도 고려** |
...
**EMS:** 운영·장애 대응을 위해 **관측(EMS) 구축이 필요**하나, 본 최소 구성 **§2 아키텍처 도식에는 그리지 않는다**. 서버 대수·최소 스펙은 §4.2에서 **STT·NLP·LLM과 동일한 2+TB1(3대)** 패턴으로 둔다. **CAPEX·초기 SW(MM) 금액은 본 문서에서 산출하지 않으며「고려 필요」** 로 표기한다(외부·전사 공용 관제 대체 여부 포함).
...
| 서버 종류 | 운영 | TB | **합계** | 이중화·비고 | 권장 스펙(노드당, 요약) | 노드당 ROM | 합계 ROM |
|-----------|------|-----|----------|-------------|-------------------------|------------|----------|
| **EMS** *(관측·별도 구역)* | 2 | 1 | **3** | 운영 All-Active + TB 1 | **최소** 16 vCPU, 64 GB RAM, HDD 1 TB, 1 Gbps *(AI Runtime과 동일 티어)* | **고려 필요** | **고려 필요** |

| 항목 | 본 문서 처리 |
|------|----------------|
| **§2 아키텍처** | EMS **미표기** (코어·AI·연동만) |
| **관측 대상** | AI Runtime, STT, NLP·LLM 및 연동 구간(OTel 등 — 상용 §8 참고) |
| **CAPEX ROM** | **미산정 · 고려 필요** (§11.2 단가·보존기간·로그량에 따라 변동) |
| **EMS SW·구축 MM** | **미산정 · 고려 필요** (§5 범위 밖) |
| **대안** | 전사·외부 공용 관제로 **대체** 시 위 3대·EMS SW는 **0**으로 재검토 |

> **EMS 3대** 는 **「도입 비용 요약」본편 합계(3.71억)·§4.1 본편 CAPEX(1.45억)에 포함하지 않는다.** STT와 같이 **필수 여부·금액은 발주 전 확정**한다.
...
| 구분 | MM | 비고 |
|------|-----|------|
| **AI Call Agent (본편)** | **8.0** | Runtime **4** + NLP·LLM **4** (§5.1) |
...
|------|-----|----------------|------------|
| **AI Runtime** *(API·세션 통합)* | **4.0** | **#1** 폭언 오케스트레이션·API 처리 **2.0** · **#2** 자막 릴레이·WSS/REST **2.0** | **약 0.52억 원** |
...
| 항목 | MM | 단가 | 금액 (ROM) |
|------|-----|------|------------|
| AI Call Agent SW (본편 HW 6대·§1) | **8.0** | 약 1,300만 원/MM | **약 1.04억 원** |

> AI 측 **WTIMS·바이토·유엔젤 연동 클라이언트·규약**은 위 8 MM에 포함한다(폭언·API **2 MM**·자막 **2 MM** 범위). **기존 제품 코드 변경**은 §5.3만 반영한다.
...
| 3 | **통화매니저 API (바이토)** · **PC Client** | **실시간 STT** 수신·**PC Client 자막** 표시 | **3.0** | **약 0.39억 원** | **폭언 시 별도 담당자 알림**은 **기구현** — MM·금액 **산정 제외** |
| | **기존 노드 소계** | | **4.0** *(+ WTIMS 0.7억)* | **약 1.22억 원** | 유엔젤 1 MM + 바이토·PC 3 MM + WTIMS |

### 5.4 초기 SW 개발비 합계

| 구분 | ROM |
|------|-----|
| AI Call Agent SW (§5.2) | **약 1.04억 원** |
| 기존 노드·연동 (§5.3) | **약 1.22억 원** |
| **SW 개발 합계 (본편)** | **약 2.26억 원** |
...
| 기능 영역 | §1 | 담당 | MM | 비고 |
|-----------|-----|------|-----|------|
| WTIMS↔Runtime↔STT 세션·RTP·자막 릴레이·WSS/REST | #1, #2 | AI Runtime | **2.0** | |
...
| 기능 | 6종 | **2종** (#1, #2) |
| DB 서버 | 2대 | **없음** |
| EMS | 상용 다수 노드 | **§2 미표기** · CAPEX **3대(2+TB1) 고려 필요** |
| AI MM | 55 | **8** (STT MM 별도) |
| AI Runtime MM | 19 | **4** |
| NLP·LLM MM | 12 | **4** (폭언만) |
| 공통·검증 MM | 10 | **0** (서버별 포함) |
| 유엔젤 연동 | CM+호제어 등 | **WTIMS 릴레이 1 MM** (호제어 기구현) |
| 바이토·PC | 자막+폭언 알림 | **자막 3 MM** (폭언 알림 기구현) |
