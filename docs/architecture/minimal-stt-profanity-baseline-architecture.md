...
| 구분 | ROM | 상세 |
|------|-----|------|
| **CAPEX** (본편 HW **4대**) | **약 0.97억 원** | AI Runtime 2 + NLP·LLM 2 (§4.1, 권장 **0.8억 ~ 1.2억**, ±20%) |
| **CAPEX** (STT **별도**, 2대) | **약 0.60억 원** | §4.2 — **합계 미포함** |
| **초기 SW 개발** (본편) | **약 3.05억 원** | AI Call Agent **1.69억** (§5.2) + 기존 노드 **1.36억** (§5.3) |
| **합계** (STT CAPEX 제외) | **약 4.02억 원** | 권장 **약 3.7억 ~ 4.4억 원** |
| *(참고) STT CAPEX 포함 시* | *약 4.62억 원* | *본편 합계 + STT HW만* |
...
|-----------|-----------|--------|------|---------|------------|
| **AI Runtime** *(API·세션 통합)* | **2** | All-Active | WTIMS 호·STT 세션, 전사 릴레이, 폭언·자막 오케스트레이션, 바이토/유엔젤 클라이언트 | #1, #2 | **포함** |
| **STT** | **2** | All-Active | RTP 직연·전사(자막·폭언 입력) | #1, #2 | **별도** §4.2 |
| **NLP·LLM** *(동일 호스트)* | **2** | All-Active | 1차 NLP·2차 LLM(**폭언·욕설 확정만**) | #1 | **포함** |

**물리 합계: 6대** (본편 **4대** + STT **2대** 별도).

### 3.2 성능·용량 기준 (가상 시나리오)

[production-deployment-architecture.md](./production-deployment-architecture.md) **§4·§5·§6.1** 을 **3종·6대(DB 제외)** 기준으로 요약한다. STT·Runtime 벤치는 확장안과 동일하고, **LLM 부하는 폭언 2차 확정 위주**로 확장안보다 작을 수 있다(스테이징에서 재확인).
...
### 4.2 STT 서버 (별도 산출 — 합계 미포함)

| 서버 종류 | 구성 대수 | 이중화 | 권장 스펙(노드당, 요약) | 노드당 ROM | 합계 ROM |
|-----------|-----------|--------|-------------------------|------------|----------|
| **STT** | **2** | All-Active | 32 vCPU, 128 GB RAM, **L40S×1**, HDD 2 TB, 10 Gbps | 약 2,995만 원 | **약 5,990만 원** |

> STT는 **별도 과제·예산**으로 발주한다. 본 문서 **§2·§3** 연동·용량은 STT 구축 시 **필수 전제**이다. STT **SW 개발 MM** 은 §5.1에서 **항목만 표기**하고 금액 합계에는 넣지 않는다.
...
| 구분 | MM | 비고 |
|------|-----|------|
| **AI Call Agent (본편)** | **13.0** | Runtime **9** + NLP·LLM **4** (§5.1) |
...
|------|-----|----------------|------------|
| **AI Runtime** *(API·세션 통합)* | **9.0** | **#1** 폭언 오케스트레이션·바이토/유엔젤 **5.0** · **#2** 자막 릴레이·WSS/REST **3.0** · **연동·장애처리** **1.0** | **약 1.17억 원** |
| **STT** | **—** | **#1, #2** 입력(전사) — **별도 과제** | *(본편 합계 제외)* |
| **NLP·LLM** *(동일 호스트)* | **4.0** | **#1** 1차 NLP **1.0** · 2차 LLM 폭언 확정 **3.0** | **약 0.52억 원** |
| **합계 (본편)** | **13.0** | | **약 1.69억 원** |

### 5.2 AI Call Agent 개발비 ROM

| 항목 | MM | 단가 | 금액 (ROM) |
|------|-----|------|------------|
| AI Call Agent SW (본편 4대·§1) | **13.0** | 약 1,300만 원/MM | **약 1.69억 원** |

> AI 측 **WTIMS·바이토·유엔젤 연동 클라이언트·규약**은 위 13 MM에 포함한다. **기존 제품 코드 변경**은 §5.3만 반영한다.
...
| 2 | **통화매니저 API (유엔젤)** | 실시간 STT **필요 호**에 한해 **시그널·세션 정보를 WTIMS로 연동 전달** | **0.1** | **약 130만 원** | **안내 방송·호 종료**는 **기구현** — 본 과제 범위 외. **서비스 가입(청약) 처리**는 **미산정** |
| 3 | **통화매니저 API (바이토)** · **PC Client** | **실시간 STT** 수신·**PC Client 자막** 표시 | **5.0** | **약 0.65억 원** | **폭언 시 별도 담당자 알림**은 **기구현** — MM·금액 **산정 제외** |
| | **기존 노드 소계** | | **5.1** *(+ WTIMS 0.7억)* | **약 1.36억 원** | 유엔젤 0.1 MM + 바이토·PC 5 MM + WTIMS |

### 5.4 초기 SW 개발비 합계

| 구분 | ROM |
|------|-----|
| AI Call Agent SW (§5.2) | **약 1.69억 원** |
| 기존 노드·연동 (§5.3) | **약 1.36억 원** |
| **SW 개발 합계 (본편)** | **약 3.05억 원** |
...
| 기능 영역 | §1 | 담당 | MM | 비고 |
|-----------|-----|------|-----|------|
| WTIMS↔Runtime↔STT 세션·RTP 연동·자막 릴레이 | #1, #2 | AI Runtime | **3.0** | |
| 폭언 이벤트·바이토 수집·유엔젤 트리거 | #1 | AI Runtime | **5.0** | 기존 호제어 호출 |
| API·WSS·바이토·PC(자막) | #2 | AI Runtime | **1.0** | Runtime 9 MM 내 잔여 연동 |
...
| 기능 | 6종 | **2종** (#1, #2) |
| DB 서버 | 2대 | **없음** |
| AI MM | 55 | **13** (STT MM 별도) |
| AI Runtime MM | 19 | **9** |
| NLP·LLM MM | 12 | **4** (폭언만) |
| 공통·검증 MM | 10 | **0** (서버별 포함) |
| 유엔젤 연동 | CM+호제어 등 | **WTIMS 릴레이 0.1 MM** (호제어 기구현) |
