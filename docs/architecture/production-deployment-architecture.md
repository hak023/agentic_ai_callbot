...
| 항목 | 내용 |
|------|------|
| 용량 목표(가상 시나리오) | 가입자 15,000명/시간, **약 4.17 CPS(운영 기준)**, 평균 통화 유지 120초 |
| 동시세션 산정(가상 시나리오) | `LLM 2노드(fp8 1,500 tok/s/노드) 병목 기준 ≈ 100 동시 AI 호 (전체 500 동시호) 수용 가능` |
...
아래는 **EMS를 제외한** 가상 시나리오(유저당 1시간 1통화, 120초 유지, 5초당 발화, AI 비율 20%)를 기준으로 한 **아키텍처·서버·비용 요약**이다. 용어·경계·연동 규격·체크리스트·상세 비용 근거는 **뒤쪽 절**을 참조한다.

**읽는 순서(권장):** (1) 본 빠른 참조 → (2) 설계 전제(1절)·전체 배포도(2절) → (3) 프로토콜 요약(3절) → (4) 시퀀스·역할 스펙(4~6절) → (5) DB·EMS·체크리스트·결론(7~10절) → (6) 비용 상세(11절: **하드웨어 CAPEX**·**AI Call Agent SW 개발(PRD)**·기존 노드 연동·외부 STT 참고) → (7) **부록 A** 연동 예제.
...
| AI Call Agent | PostgreSQL HA | 2 + 0 / 2 + 1 |
| AI Call Agent | Qdrant(VectorDB) | 2 + 0 / 3 + 0 |
...
### 빠른 참조 — 용량·수용량 (벤치 한계 vs §6.4 시나리오)

- **§6.4 가상 시나리오:** 유저당 1시간에 1통화, 통화 120초, 5초당 1회 발화(호당 24회), 전체 호 중 AI 응대 비율 20%(8:2).
- **벤치 한계:** 노드·클러스터별로 **역할에 맞는 단위**(동시 ASR 스트림, 합성 슬롯, inf QPS, 오케스트레이션 세션, TPS 등)를 **부하 테스트로 측정한 상한**(또는 §5 목표치).
- **시나리오 부하:** 위 §6.4에서 **그 노드가 실제로 받는 부하**만 환산한다. **가용 여유 판단 = 벤치 한계 대비 시나리오 부하**(단위 일치).
- 산식·가정·RAG 빈도·DB 트랜잭션 → **§5**, **§6.4**, **§7** / 체크리스트 **§6.2**.

| 구분 | 벤치·클러스터 상한 (측정·§5 목표) | §6.4 BH 시나리오에서의 기대 부하 | 비고 |
|------|-----------------------------------|----------------------------------|------|
| LLM | **25** inf QPS (120 tok/req, 2노드 합); 운영 **80% → 20** QPS | **20** QPS (AI 호당 0.2 QPS) | AI 구역 병목; **동시 AI 통화 100** 수용 가능 |
| AI Runtime | **500** 동시 세션(250/노드×2, 벤치) | **100** 동시 세션(활성 AI 호 1:1) | 오케스트레이션 단위 (여유) |
| STT | **1,250** 동시 스트림(625/노드×2) | **100** 동시 스트림(호당 1 ASR RTP mirror 레그) | 미디어 평면 (큰 여유) |
| TTS | **2,000** 동시 합성(1,000/노드×2) | **≤100** 동시(턴·합성 겹침 피크, 보수) | §5.2; (큰 여유) |
| API/Realtime | 동시 WSS **5,000** (Active 1대 벤치 가정, §5.5) | **WSS:** 연동 모델(통화와 1:1 아닐 수 있음) | §1.6 Active 1대 인바운드 산정 |
| WTIMS RTP | **800** 세션/노드(벤치; Active 1 기준 예) | **500** 동시 RTP(전체 호 100%) | 코어 미디어 앵커 (여유) |
| PostgreSQL | **TPS·커넥션** 벤치(§7) | **쓰기/읽기**는 세션·이력·RAG 경로별 §6.4·§7에서 환산 | 풀 한도 별도 |
| Qdrant | 클러스터 **~200** QPS(보수 벤치) | **§7.2** 호당 조회·업서트 빈도 × 동시 통화 | RAG 평면 |

### 빠른 참조 — 노드별 가용 용량 매트릭스

열 **「벤치 상한」** 은 노드 역할에 맞는 단위로 측정한 **천장**, **「§6.4 BH 부하」** 는 가상 시나리오(1콜/h·120s·5s 발화·AI 20%)에서의 **해당 노드 기대치**다. 
**최대 수용 AI 동시호**와 **최대 수용 가입자**는 **해당 노드의 벤치 상한만으로 역산했을 때** 수용 가능한 이론적 한계치다. 이를 통해 **어느 노드가 전체 시스템의 병목인지(LLM과 WTIMS)** 파악한다.

| 노드 | 단위 | 벤치 상한 (클러스터·요약) | §6.4 BH 시나리오 기대 부하 | 최대 수용 AI 동시호 (단독 한계) | 최대 수용 전체 동시호 (단독 한계) | 최대 수용 가입자 (명, 단독 한계) | 대수 |
|------|------|---------------------------|---------------------------|---------------------------------|-----------------------------------|----------------------------------|------|
| LLM | inf QPS | **25** (운영 **20**) | **20** | **100** | **500** | **15,000** | 2 |
| AI Runtime | 동시 세션 | **500** | **100** | **500** | **2,500** | **75,000** | 2 |
| STT | 동시 스트림 | **1,250** | **100** | **1,250** | **6,250** | **187,500** | 2 |
| TTS | 동시 합성 | **2,000** | **≤100** (피크) | **2,000** | **10,000** | **300,000** | 2 |
| API/Realtime | 동시 WSS | **5,000** (가정, §5.5) | 연동 모델 의존 | **5,000** (1:1 가정 시) | **25,000** | **750,000** | 1+1 |
| AIR GW | 시그널 TPS | 제품·프로파일 벤치 | **~4.2** CPS권 + 세션 이벤트율 | — | — | — | 1+1 |
| WTIMS RTP | 동시 RTP | **800**/Active(예) | **500** (전체 호) | **160** | **800** | **24,000** | 1+1 |
| PostgreSQL | TPS 등 | §7 벤치 | §6.4·§7 트랜잭션·풀 모델 | — | — | — | 2 |
| Qdrant | QPS | **~200** 클러스터 | §7.2 호당 빈도 × 동시 부하 | **~1,000** (호당 0.2 QPS 시) | **5,000** | **150,000** | 2 |

서버 역할·스펙 표: **§6**. AIR GW **TPS·매핑**, PostgreSQL **AIR 외 산정**, Qdrant **RAG 빈도** 등 세부는 본문 **§5~§7** 참조.

### 빠른 참조 — 비용 요약 (하드웨어 CAPEX + SW 개발)

| 구분 | 금액(요약) | 상세 |
|------|------------|------|
| **하드웨어 CAPEX** | AI Call Agent **16노드(본 시나리오)** 합계 약 **333,910,000원** (ROM, HDD 기준); 현실 범위 **약 2.7억 ~ 4.0억원**(±20%) | 11.2절 기준 |
| **AI Call Agent SW 개발 (순수 개발비)** | [prd.md](../product/prd.md) 기준 **72.0 MM** → **약 936,000,000원 (약 9.36억원, ROM)** — 역할별 분해는 **11.4절** | HW CAPEX·§11.3 연동과 구분 |
| **기존 노드·외부 연동 SW 개발** | WTIMS 경로 **약 0.7억원**; CM↔WT·유엔젤/바이토↔API·EMS 등 별도 산출 | **11.3절** (본체와 이중 계상 금지) |
...
- **동시세션(운영 기준)**: 500호 (전체 호), 100호 (AI 호) (Busy Hour 기준)
- **CPS**: 약 4.17 INVITE/s 지속 유입
- **AI 적용률 가정**: 20% (전체 호 중 20%만 AI 응대)
- **실시간 AI 활성 구간 가정**:
  - STT 활성 세션: `100` (AI 호 기준 상시 점유)
  - TTS 동시 합성 기준: 서버 처리량 기준으로 산정(`1,000 세션/노드`, 1 Active = 1,000)
  - LLM 동시 생성 요청: §6.4 계산식 기준(발화 빈도·질의수 가정)
...
본 설계·**비용 산출(11절)** 에는 **L4/L7 로드밸런서 전용 장비(CAPEX)** 를 포함하지 않는다. **통화매니저 API 서버(유엔젤·바이토)** 가 API/Realtime을 호출할 때는 **단일 접점 VIP/FQDN 1개**로만 수렴시키되, VIP는 **정상 시 Active 노드**에 부착하고 **Standby**는 동일 스펙으로 대기한다. 장애 시 VIP 전환(예: Keepalived/VRRP)·DNS 스위치·운영 수동 전환 등으로 Standby가 인수한다. 정상 시 **동시 WSS·REST 수용량**은 **Active 1대 기준**으로 **벤치 상한**을 두고, **§6.4 시나리오에서 기대되는 rps·WSS**와 대조해 여유를 본다(§6.2).
...
- **구성**: 본 시나리오 **2노드 All-Active** → 클러스터 **1,250 동시**(벤치 합산). (시나리오 요구량 100 대비 충분한 여유)
...
- **구성**: 본 시나리오 **2노드 All-Active** → 클러스터 **2,000 동시**(벤치 합산). (시나리오 요구량 100 대비 충분한 여유)
...
- **QPS 및 동시 call 산정 (클러스터 2노드)**
  ...
  - 운영 동시 call(권장): `20 / 0.2 = 100` → **100 AI call** (전체 500 call)
  - 본 문서의 기준 용량은 **운영 기준 100 동시 AI 세션**으로 둔다.
...
- **처리량 가정 (벤치):** **동시 WSS·REST rps** 상한은 **부하 테스트로 확정**(Active 1대, §1.6). (예: 단일 노드 5,000 WS 이상)
- **시나리오 부하 (§6.4):** 코어 **~4.2 CPS**·유엔젤·바이토 **연동 모델**(호마다 장기 WSS 여부, 집계 채널 여부)로 **인바운드 rps·동시 WSS**를 따로 환산한다. 빠른 참조 **매트릭스·본 절 §6.2**와 맞춘다.
...
## 6) 서버 산정표 (§6.4 시나리오 부하 vs 벤치 상한)

**제목의「250」:** §6.4에서 LLM 병목으로 역산한 **동시 활성 AI 통화 목표**이며, **모든 노드의 벤치 단위를 250으로 맞추는 값은 아니다**(빠른 참조 매트릭스·§6.2 참조).

**EMS 제외 구성·스펙·용량 요약:** 문서 앞쪽 **빠른 참조** 절. **벤치 상한 vs §6.4 시나리오 부하·서버 대수:** 문서 앞쪽 **「노드별 가용 용량 매트릭스」**. 아래 표는 **EMS 포함** 전체 산정이며, 옵션은 6.3절.

| 구분 | 계층 | §6.4 시나리오 기대 부하 (역할별) | 벤치·노드당 상한 (측정·§5) | 필요 Active 노드 | 권장 구성(여유 포함) |
|------|------|----------------------------------|---------------------------|------------------|----------------------|
| 기존 코어 | 통화매니저AS | 기존 코어 사용 | 기존 Pair 용량 기준 | 기존 2 Pair 활용 | 추가 구축 없음 |
| 기존 코어 | WTIMS RTP | **500** 동시 RTP(전체 호) | **800** 세션/노드(벤치) | 1 | 1 Active + 1 Standby |
| AI Call Agent 시스템 | AIR 연동 접점 GW | **~4.2** CPS권 시그널 + **100** 동시 호 컨텍스트 이벤트 | 제품·프로파일 **TPS 벤치** | 1 | 1 Active + 1 Standby |
| AI Call Agent 시스템 | STT | **100** 동시 스트림(호당 1 ASR 레그) | **625**/노드(벤치) | **2** | **2 Active** (All-Active; 클러스터 **1,250** 동시) |
| AI Call Agent 시스템 | TTS | **≤100** 동시 합성(턴 피크, 보수) | **1,000**/노드(벤치) | **2** | **2 Active** (All-Active; 클러스터 **2,000**) |
| AI Call Agent 시스템 | LLM | **20** QPS (80% 운영) | `1,500 tok/s x 2노드`, 120 tok/req → **25** QPS 벤치 | 2 | **2 Active (fp8)** |
| AI Call Agent 시스템 | AI Runtime | **100** 동시 세션(오케스트레이션) | **250**/노드(벤치) | **2** | **2 Active** (All-Active; GW 해시; 클러스터 **500** 동시 벤치) |
| AI Call Agent 시스템 | API/Realtime | **§6.4·연동 모델:** 코어 **~4.2 CPS** 기반 트리거 **rps** + **WSS**는 호 1:1 구독 여부에 따라 의존 | **5,000 WS** (가정, §5.5 참조) | 1 | **1 Active + 1 Standby** · 단일 VIP §1.6 |
| AI Call Agent 시스템 | PostgreSQL HA | 세션·이력·RAG 경로별 **TPS** (§6.4·§7) | 쓰기 Primary·풀 **벤치** | 2 | **최소** Primary + Standby(HA) |
| AI Call Agent 시스템 | Qdrant(VectorDB) | **§7.2** 호당 조회·업서트 × 동시 부하 + 10만 고객 지식베이스 | **100** QPS/노드(보수 벤치) | 2 | **최소 2 Active** |
...
> 상기 수치는 초기 계획치이며, 반드시 스테이징에서 **4.17 CPS/120초/500 동시세션 (AI 100)** 부하로 재검증한다.
...
- 시나리오 가정:
  - 유저당 통화 1회 `120초` 유지
  - 유저 발화 `5초당 1회` → 통화당 `24회`
  - 발화당 LLM 질의 `1회` → 통화당 `24회`
  - 유저는 `1시간에 1회 통화`
  - 전체 통화 중 **AI 응대 비율 20% (유저간 통화 80%)**
- LLM 2노드(fp8 1,500 tok/s/노드), 120 tok/req 기준:
  - 이론 QPS: `(1,500 x 2) / 120 = 25 req/s`
  - 운영 QPS(80%): `20 req/s`
  - AI 호 1개당 LLM QPS: `1 req / 5s = 0.2 QPS/호`
  - **최대 수용 AI 동시호 (LLM 병목 시):** `20 QPS / 0.2 QPS/호 = 100 동시호`
- 전체 통화 및 가입자 환산 (AI 비율 20% 적용):
  - **최대 수용 전체 동시호:** `100 / 0.2 = 500 동시호`
  - 시간당 처리 가능한 전체 통화 수 (CPS 환산): `500 동시호 x (3600 / 120) = 15,000 콜/시간` (약 **4.17 CPS**)
  - **시간당 수용 가능한 가입자 수(유저당 1콜/시간 가정): `15,000명/시간`**

*(참고: 위 계산은 **LLM이 시스템 전체의 병목**이라고 가정했을 때의 수용량이다. 각 노드별 단독 한계는 앞의 매트릭스 표를 참조한다.)*
...
| AI Call Agent 시스템 | API/Realtime (단일 VIP) | 1 + 1 | 16 vCPU | 32 GB | - | HDD 1 TB | 1 Gbps | REST/WSS · 벤치 상한은 §5.5; 시나리오 rps·WSS는 §6.4·연동 모델 |
...
### 6.2 목표 용량 대비 계산 체크 (벤치 vs §6.4 시나리오)

- **시나리오 축:** §6.4 가상 시나리오에서 **동시 활성 AI 통화 100호 (전체 통화 500호)** 은 **LLM 병목 역산 결과**이며, STT·AIR 등은 그에 따른 **파생 부하**다.
- **LLM:** 시나리오 **20 QPS** / 벤치 **25 QPS** → 운영 **80%** 규칙 내 여유.
- **STT:** 시나리오 **100** 동시 스트림 / 벤치 클러스터 **1,250** → 큰 여유.
- **TTS:** 시나리오 **≤100** 동시 합성(보수) / 벤치 **2,000** → 여유(피크는 부하로 재확인).
- **AI Runtime:** 시나리오 **100** 세션 / 벤치 클러스터 **500** → 한 대 장애 시에도 단기 이월 가능(해시 재균형).
- **WTIMS RTP:** 시나리오 전체 호 **500** / 벤치 **800**/Active(예) → 여유.
- **API/Realtime:** 시나리오 **rps·WSS**는 **CPS·연동 모델**로 별도 환산; 벤치 상한은 **§5.5**에서 확정. (예: 5,000 WS 벤치 시 큰 여유)
- **PostgreSQL·Qdrant:** 시나리오 **TPS·QPS**는 **§7·§7.2** 환산; 벤치 상한과 비교해 풀·샤딩을 맞춘다.
...
**한눈에 요약(EMS 제외 HW·SW + 외부 STT 참고):** 문서 앞쪽 **빠른 참조 — 비용 요약**을 본다. 아래는 단가 가정·역할별 합산·**PRD 연계 AI Call Agent 개발비**·연동 개발 항목·외부 STT 비용·출처의 **상세**다. **월간 유지보수·전력·랙 등 가동 OPEX** 는 본 문서 **범위에 포함하지 않는다.**

본 절은 (1) **AI Call Agent 시스템** **하드웨어 CAPEX**(11.1·11.2), (2) **AI Call Agent 시스템 소프트웨어 순수 개발비**(11.4 — [prd.md](../product/prd.md) MM), (3) **AI Call Agent를 제외한** 기존 노드·외부 연계 **연동 개발**(11.3), (4) **외부 STT 등 타사 솔루션 참고 견적**(11.5), (5) **하드웨어 단가 출처**(11.6)를 구분한다.  
**용어:** **「SW 개발」** 은 **인월 기반 개발 ROM**(11.4)을 가리킨다. 개발 항목은 산출된 것만 금액을 표시하고, 미산출은 **항목·범위**만 둔다.
...
> 범위 권고: 부품·플랫폼 ROM·리셀러 할인·번들 조건에 따라 실제 발주가는 보통 **±20%** 편차가 발생한다.  
...
[prd.md](../product/prd.md) **개발 공수 (MM)** 는 **AI Call Agent 시스템**에 대한 **설계·구현·품질 개런티·다단계 통합 검증**을 포함한 **개발 인월**이다. 본 절의 금액은 **그에 대응하는 소프트웨어 개발비 ROM**이며, **하드웨어 CAPEX(11.2)**·**11.3절 기존 노드 연동**과 **이중 계상하지 않는다**.
...
| **합계 (AI Call Agent SW)** | **72.0** | **936,000,000 (약 9.36억원)** |

**AI Runtime MM 상향 검토(ROM):** 기존 **15.0 MM** 은 세션 오케스트레이션·Intent·HITL·추론 클라이언트 중심이었으나, **Pipecat**(또는 동등) 기반 **실시간 음성 파이프라인**(프레임/프로세서·트랜스포트·턴테이킹·바지 인터럽션·STT/TTS 스트림 정합), **Intent·업무 시나리오별** 모듈·상태·프롬프트 분리 및 **시나리오 단위 회귀**, **`call_id` 고정·GW 해시**와 맞춘 세션 생애주기 검증 등에 **추가 인월**이 필요하다고 판단하여 **+7.0 MM → 22.0 MM**으로 조정했다. (PRD **v3.6** 서버 역할 표·Epic 분해와 동일 합계 **72.0 MM**.)
...
| 서버 영역 | 벤더/옵션 | 비용 성격(CAPEX·라이선스·개발 등) | 금액 | 산출 기준(채널/노드/토큰 등) | 기준일 | 상태 |
|-----------|-----------|-------------------------------------|------|-------------------------------|--------|------|
| STT | 예: Vendor A | 라이선스·연동개발 | TBD | 예: 300ch, 12개월 | YYYY-MM-DD | 예정/확정 |
