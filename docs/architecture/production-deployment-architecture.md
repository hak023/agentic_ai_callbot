...
- 엔드투엔드 한도는 **병목 계층**에 맞춘다.
- 아래는 **벤치 또는 벤치로 검증한 설계 목표**만 적는다. 산식·가정·병목 논의 → **§5**, **§6.4**, **§7** / 체크리스트 **§6.2**.

| 구분 | 벤치·설계 (Active 합) |
|------|------------------------|
| STT | **625** 세션/노드 × **2** → 동시 **1,250** |
| TTS | **1,000**/노드 × **2** → **2,000** |
| LLM | **1,500** tok/s/노드(fp8, 벤치) × **2** → **3,000** tok/s; 120 tok/req 시 클러스터 **25** QPS(운영 **80% → 20** QPS) |
| AI Runtime | **250** 세션/노드 × **2** → **500** |
| API/Realtime | REST **80** rps · WS **250** 동시(설계·벤치); 가입자 환산 → **§6.4** |
| WTIMS | **800** RTP/노드 × **1** → **800** |

### 빠른 참조 — 노드별 가용 용량 매트릭스

성능·동시 한도는 **벤치·부하테스트 확정값**. CPS·명/h 열은 평균 호 **120s**·**1콜/h** 등 **§6.4** 참조 가정. API/Realtime 가입자는 REST·WS 벤치와 가입자당 사용량 가정으로 **§6.4**.

| 노드 | 벤치·설계 성능 | CPS_max (120s 호) | 동시 점유 | 명/h (참조) | 대수 |
|------|----------------|---------------------|-----------|-------------|------|
| AIR GW | 시그널 **TPS 벤치** | 목표 **≥~2.1** INV/s | 동시 시그널 벤치 | §매핑 | 1+1 |
| API/Realtime | REST **80** rps · WS **250** 동시 (벤치·설계) | §6.4 | **250** | §6.4 | 1+1 |
| AI Runtime | 벤치 | **≈4.17** | **500** | **15,000** | 2 |
| TTS | 벤치 | **≈16.67** | **2,000** | **60,000** | 2 |
| STT | 벤치 | **≈10.42** | **1,250** | **37,500** | 2 |
| LLM | **25→20** QPS(벤치·80%) | **≈2.08** | **250** | **7,500** | 2 |
| PostgreSQL | **TPS 벤치** | 부하테스트 | 풀 한도 벤치 | § | 2 |
| Qdrant | 노드 **100** / 클러스터 **~200** QPS(벤치) | §호당 빈도 | §7.2 시나리오 | §RAG | 2 |
...
| **AI Call Agent SW 개발 (순수 개발비)** | [prd.md](../product/prd.md) 기준 **72.0 MM** → **약 936,000,000원 (약 9.36억원, ROM)** — 역할별 분해는 **11.4절** | 월간 유지·전력과 구분 |
